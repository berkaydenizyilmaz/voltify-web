...
const CITIES = [
  ...
  { name: "Bursa", lat: 40.1885, lon: 29.061, weight: 0.09 },
  { name: "Antalya", lat: 36.8969, lon: 30.7133, weight: 0.08 },
  { name: "Adana", lat: 37.0, lon: 35.3213, weight: 0.06 },
...
// Returns data starting from current hour for the next 7 days
export async function getWeatherForecast(
  ...
  for (let i = 0; i < hourCount; i++) {
    allWeather.push({
      ...
      shortwave_radiation: calculateWeightedAverage(
        citiesData,
        i,
        "shortwave_radiation"
      ),
      weather_code: Math.round(
        calculateWeightedAverage(citiesData, i, "weather_code")
      ),
    });
  }
