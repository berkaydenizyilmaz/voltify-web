...
async function getTgt(): Promise<string> {
  ...
  // Get TGT from EPİAŞ auth endpoint
  const response = await post<TgtResponse>(EPIAS_AUTH_URL, {
    username,
    password,
  })
...
export async function getUecm(startDate: Date, endDate: Date): Promise<UecmResponse> {
